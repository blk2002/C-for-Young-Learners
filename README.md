...
除了小程序本身，仓库还包含一个给老师用的**网页版课程维护工具**（`content-workbench/`），用来解决小程序端逐题手填课程和题库太慢的问题。

### 为什么需要它

小程序里维护课程结构、逐题录入习题，效率很低。工作台让老师在电脑浏览器里批量完成这些工作，再一键同步到云数据库，小程序端直接读取，无需在手机上反复点。

### 核心功能

- 学科 / 章节 / 知识点三级结构：手动增删改，也可粘贴素材让 AI 生成
- 知识点三段速记：概念、特征、易混淆点分开维护，附代码示例
- 三类题目：
  - 学习题（内嵌在知识点 `lessons.questions`）、章节题（`chapterQuestions`），随知识点 / 章节存在
  - 考试题（`examQuestions`）：独立实体，按「学科 → 考试类型（CIE / GESP / CSP-J/S）→ 级别」分类批量维护，支持手动加一道与「AI 按本分类生成」
...
- 云函数：`importContent`（写入）、`getCourseTree`（读取），均带 `ADMIN_PASSWORD` 密码校验

### 部署

工作台的部署方式、云函数权限配置与小程序端不同，单独见 `content-workbench/DEPLOYMENT.md`，含完整分步说明和常见踩坑记录。
