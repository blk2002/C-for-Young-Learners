...
> **内置学科**：`python`（Python）和 `cpp`（C++）是内置学科，**不可删除、不可改名**。内置学科写在 `js/state.js` 的 `BUILTIN_COURSES`；每次 `load()` 都会兜底补齐（缺哪门补哪门，只补空壳，已有学科与内容不动，重复打开不会重复插入）。
>
> **学科管理职责**：学科的「新建 / 删除 / 改名」由管理员在**小程序首页**完成（写入云端 `courses` 集合）；工作台**只编辑内容**（章节 / 知识点 / 习题），点「从云端拉取」会把云端学科 registry 合并进左侧学科列表（含小程序端新建的空学科）。
...
- 新学科：管理员在小程序首页「新建学科」创建，工作台「从云端拉取」自动出现
