...
· 新学科小程序不显示：学科由管理员在小程序首页「新建学科」创建，工作台点「从云端拉取」即可看到
· 新学科的考试页是空的：新学科默认没有等级考试类型，去题目页「⚙ 考试类型配置」添加并保存
    ...
    steps: [
      {
        ...
        lines: [
          ...
          '工作台只编辑内容；学科的「新建 / 删除 / 改名」由管理员在小程序首页完成，点「从云端拉取」同步过来。',
  ...
  const SECTIONS = () => [
    ...
    },
    {
      ...
      lines: [
        ...
        '考试类型与等级列表在哪配：题目页「⚙ 考试类型配置」，按学科维护，保存即上云。新建学科默认没有考试类型，先去配好，小程序的考试页才会出卡片。',
    ...
    {
      kind: 'plain', title: '导出备份 / 导入备份是什么',
      tip: '右上角这两个按钮是"本地草稿的存档 / 读档"，跟云端无关。',
      lines: [
        '存的是什么：你在工作台的全部编辑（学科、章节、知识点、习题、考试题、待同步队列）都保存在当前浏览器的本地草稿里。「导出备份」把它整体存成一个 JSON 文件下载到电脑。',
        '什么时候用：换电脑、换浏览器、清浏览器缓存之前；以及做大批量删除或 AI 整理之前先导一份，操作翻车了可以导回来。',
        '注意：导入是覆盖不是合并 —— 备份文件会整体替换当前浏览器里的草稿。导入前如果当前草稿里有没备份过的内容会丢，所以导入前先点一次「导出备份」留底。',
        '和「同步到云端」的关系：两码事。已同步到云端的内容，浏览器清了可以「从云端拉取」找回来；备份真正保护的是还没同步的本地编辑和整体工作状态。'
      ]
    },
